...
in
{
  config = lib.mkIf cfg.enable {
    ...
    services.wyoming.satellite = {
      ...
      # Wrapped in sh -c because wyoming-satellite uses subprocess_exec (no shell), so
      # the pipe would otherwise be passed as a literal argument to arecord.
      microphone.command = "sh -c 'arecord -D ${cfg.satellite.micDevice} -r 44100 -c 1 -f S32_LE -t raw | sox -t raw -r 44100 -c 1 -e signed-integer -b 32 - -t raw -r 16000 -c 1 -e signed-integer -b 16 -'";
      # HDA Intel PCH (hw:0,0): convert mono 22050 Hz to stereo 48000 Hz via sox
      # VERIFY device index with `aplay -l` after install
      sound.command = "sh -c 'sox -t raw -r 22050 -c 1 -e signed-integer -b 16 - -t raw -r 48000 -c 2 -e signed-integer -b 16 - | aplay -D ${cfg.satellite.sndDevice} -r 48000 -c 2 -f S16_LE -t raw'";
