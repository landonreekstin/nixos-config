...
- [ ] **Finish the 26.05 rollout** *(PR #145 open — needs a gaming-pc reboot, then per-host rebuilds)* — PR #143 pauses `autoUpdate` on blaney-pc / optiplex-nas / mini-server and must merge **first**; PR #145 is the upgrade itself and is already switched (not rebooted) on gaming-pc. Remaining: reboot gaming-pc and verify NVIDIA on kernel 6.18, the Plasma and Hyprland sessions and a game — a `switch` cannot test this, since 26.05 makes systemd stage-1 initrd the default and swaps dbus for dbus-broker. Then soak, merge, and roll out most-recoverable first, each a deliberate rebuild **and reboot**: mini-server → optiplex-nas (physical access only; re-check the `/mnt/private` LUKS unlock and the CI runner) → the desktops → blaney-pc last, in person. Finally revert #143. Procedure in [docs/flake-updates.md](docs/flake-updates.md#release-upgrades-2511--2605-and-the-next-one).
