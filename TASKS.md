...
- [x] **Public file drop on optiplex-nas, for remote VPN users** *(done on optiplex-nas — PR [#147](https://github.com/landonreekstin/nixos-config/pull/147))* — `customConfig.homelab.publicFiles` serves `/mnt/storage/public` read-only over nginx at `http://files.lan/public/` and `http://192.168.1.76/public/`. No firewall change was needed, as predicted. The `serverAliases` trap was real and worth the runbook entry: pre-change, `curl -H 'Host: 192.168.1.76' http://127.0.0.1/` on the NAS returned **Bazarr** — `bazarr.lan` sorts first and was therefore nginx's default vhost. Verified in place: 200 on both Host values, autoindex lists the drop, PUT/POST 403, `Range` 206 (resume works), `../` out of the drop 404. Scope was HTTP only; the runbook's optional Samba instance was skipped as it adds nothing for remote peers. Both follow-ups are now done, so `docs/runbooks/blaney/01-install-mw2-iw4x.md` is unblocked: `mw2.tar` (16.5 GB — gaming-pc's installed, IW4x-synced tree) has been in the drop since 2026-09-23 and serves with `Range` support, and both that runbook and the "Reaching the media" section of `mw2-iw4x-source.md` now describe the HTTP path instead of a USB drive.
- [x] **Gamarr — an *arr for PC games and retro ROMs on optiplex-nas** *(done on optiplex-nas — PR [#144](https://github.com/landonreekstin/nixos-config/pull/144))* — Request + automated download for the one media category that had no automation. `pkgs/gamarr` builds [Gamarr](https://github.com/JeremiahM37/gamarr) v1.3.0 natively (`buildGoModule`) rather than as upstream's root-running container; `customConfig.homelab.gamarr` reuses the existing Prowlarr and Transmission, and library lands hardlinked in `/mnt/storage/games/{roms/<platform>,vault}`. Playback is deliberately out of scope — ROMs go to FPGA hardware via SD card, so **RomM was evaluated and skipped**: it is a library manager with no acquisition, and its browser-play draw is unused here. **qBittorrent joins the NAS purely as Gamarr's client** — Gamarr's `TransmissionClient` implements `AddTorrent` but nothing ever calls `GetTorrents`, so a Transmission grab is started and then never detected, imported or organised (still true on upstream `main`). Transmission keeps the media stack untouched. Three upstream bugs are patched in `pkgs/gamarr/*.patch`: `SearchVimm` never skipped a platform Vimm does not carry (so a PC search returned console ROMs); category **4050 is PC/Games** but upstream maps it to Switch for Nyaa's benefit, and since that set post-filters results, every genuine PC release was discarded; and `organizeGame` called `maybeExtractArchives` in its ROM branch but not its PC branch. Two traps cost the most time and are documented at their site: Prowlarr runs `DynamicUser=yes` so `/var/lib/prowlarr` is a symlink into `/var/lib/private` (0700 root) and the API key needs a `+`-prefixed `ExecStartPre` to read; and `ReadWritePaths` must list `storagePath` **once**, because systemd makes each entry its own bind mount and the kernel refuses hardlinks across bind mounts — `RestrictSUIDSGID` is likewise absent, since imports replicate the source mode and everything under `storagePath` is 2775, making the call `mkdirat(..., 02775)`. Media source notes in [docs/runbooks/mw2-iw4x-source.md](docs/runbooks/mw2-iw4x-source.md).
- [ ] **`dbus-broker` reload times out on every `nixos-rebuild switch`, making successful rebuilds look failed** — Since 26.05, any rebuild that touches dbus config hangs for 90 s and then returns **exit status 4** with `Reload failed for D-Bus System Message Bus`, even though the switch itself fully succeeded and the broker stays `active` throughout. Reproducible on demand with no config change at all: a bare `systemctl reload dbus-broker` takes exactly 90 s and fails. **Cause:** dbus-broker 37 ships its own unit with `Type=notify-reload`, so systemd sends SIGHUP and waits for an `sd_notify` `RELOADING=1` → `READY=1` handshake that never arrives; it gives up at `TimeoutStartSec=90s`. dbus-broker is **37 on both 25.11 and 26.05**, so this is not a dbus-broker bump — the variable that changed is systemd, 258.7 → 260.4. Matches [systemd#37515](https://github.com/systemd/systemd/issues/37515). Not yet fixed here **deliberately**: the fixes all mean overriding the unit of a critical service (forcing `Type`, adding an `ExecReload` that does the pre-37 `busctl ... ReloadConfig`, or switching `services.dbus.implementation`), and optiplex-nas is headless with physical-access-only recovery. Cosmetic but genuinely misleading — worth knowing a rebuild reporting exit 4 with only this in the log is a success.
- [ ] **Finish the 26.05 rollout** *(PR #145 merged 2026-09-23 — remaining hosts need a deliberate rebuild + reboot)* — `main` is on nixos-26.05 and `autoUpdate` is back on everywhere (PR #143 reverted). **Done:** gaming-pc and mini-server both rebuilt and rebooted clean on real hardware; vm-blaney boots clean from a fresh disk with both of blaney's desktop sessions rendering. **Remaining:** (1) **optiplex-nas has not been rebooted on 26.05** — its timer will only `switch` it, so the running kernel does not change until it restarts; do that boot deliberately rather than discovering it during an unplanned power event, since recovery is physical-access-only and it hosts the CI runner, binary cache and flake-updater. Re-check the `/mnt/private` LUKS unlock and the runner afterwards. (2) mini-server is checked out on `feat/nixos-26.05`; move it back to `main` (`git checkout main && git pull`) or its auto-update will break when that branch is deleted — then the branch can go. (3) optiplex, justus-pc, atl-mini-pc, asus-laptop, asus-m15 are built but never run on 26.05. (4) blaney-pc last, in person. Procedure in [docs/flake-updates.md](docs/flake-updates.md#release-upgrades-2511--2605-and-the-next-one).

- [x] **Weekly flake-updater has stopped producing PRs** *(root-caused; fixed by the 26.05 upgrade, PR #145)* — Root cause was not the updater: **nixos-25.11 went EOL** (last upstream commit 2026-06-30), so Hydra stopped rebuilding it, packages fell out of the cache, and `nix flake update` kept advancing the non-pinned inputs past what a frozen stable could build. The runs were firing and opening PRs; W35/W36/W37/W39 were then closed rather than merged because hosts FAILed or hit the 180-min build cap. PR #142 patched the three specific breakages, PR #145 upgrades to 26.05 and removes them. The original note follows. — The last merged update was W34 (17 Aug); there are no open `flake-update` PRs and no remote `update/*` branches (`update/2026-W35/36/37` exist only as local branches on gaming-pc). Consequence: `main`'s `nixpkgs-unstable` pin is a month stale, and because `update` on a `betaTesterHost` falls back to `main` when no `origin/update/*` branch exists, gaming-pc silently rolled *backwards* on every `unstable-override` package on 2026-09-14. That surfaced as signal-desktop refusing to open its database (fixed by the version floor in PR #139, but Signal was only the loudest symptom — everything else is equally stale). Check on the NAS: is the updater timer still firing (`systemctl list-timers`, unit logs), is it failing mid-run (see `/tmp/flake-updater-build-<host>.log`), or is it opening PRs that something then closes and deletes the branch for? Verify in place on optiplex-nas, since that is where the updater actually runs.

- [ ] **Update notification: update & rebuild action button** — Extend `modules/home-manager/services/update-notification.nix` to include a dunst action button labeled "Update & Rebuild" that runs `update && rebuild` in a terminal (or background with follow-up notification). Add a second notification on completion indicating success or failure. KDE: use `kdialog` or a `.desktop` action instead of dunstify. Gate behind the existing `cfg.enable` option.
...
- [x] **Host `landon` and `Venator class XvX` as Bedrock servers** *(done on mini-server — PR [#140](https://github.com/landonreekstin/nixos-config/pull/140))* — Two Minecraft worlds recovered from a 2013 iPod / iPhone 5c, served as independently start/stoppable BDS instances with their own game-control dashboard tiles. Needed `TRANSPORT=raknet` (BDS 1.26 defaults to `transport=nethernet`, which serves connections over WebRTC signaling and makes direct IP:port joins impossible) plus a declarative allowlist. Runbook: `docs/runbooks/minecraft-bedrock-servers.md`. Also resolves a pre-existing inconsistency: `minecraft-bedrock` is defined both by the repo OCI container and by an unmanaged `docker-compose.yml` on the host, with mismatched `/data` mounts and a colliding container name.

- [x] **Recover `landon` / `venator` terrain — `Generator 0` legacy limited worlds are fenced off by BDS** *(done; verified in game)* — **No terrain was ever lost.** Block-level comparison of the BDS-opened copies against the sources showed *zero* difference (landon 4,401,773 non-air blocks and 256 chunk columns; venator 525,188 and 458 — identical before and after). The real cause: landon is `Generator 0`, a *legacy limited world*. BDS 1.26 relocates the spawn of such a world (`SpawnX/Z` 128/128 → 580/4), computes the playable boundary around that new spawn, and then rejects every coordinate in the real 0–255 build area as "outside of the world" — while refusing `setworldspawn` with "cannot be set in legacy worlds". The chunks sit on disk, unreachable. **Fix: patch one int in `level.dat`, `Generator 0 → 1` (infinite), before BDS opens the world.** BDS then preserves the spawn at 128/64/128 and generates no stray chunks. Two red herrings that cost a full session: `SpawnY=32767` is Bedrock's *auto-pick-the-surface* sentinel, and `limitedWorldWidth/Depth=16` is counted in **chunks** (16×16 = 256 blocks, correct) — both appear in the pristine, never-BDS-opened iPhone XR copy of venator, so neither is evidence of damage. Venator needed nothing: `Generator 2`, spawn never touched.
