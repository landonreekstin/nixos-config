...
- [x] **Public file drop on optiplex-nas, for remote VPN users** *(done on optiplex-nas — PR [#147](https://github.com/landonreekstin/nixos-config/pull/147))* — `customConfig.homelab.publicFiles` serves `/mnt/storage/public` read-only over nginx at `http://files.lan/public/` and `http://192.168.1.76/public/`. No firewall change was needed, as predicted. The `serverAliases` trap was real and worth the runbook entry: pre-change, `curl -H 'Host: 192.168.1.76' http://127.0.0.1/` on the NAS returned **Bazarr** — `bazarr.lan` sorts first and was therefore nginx's default vhost. Verified in place: 200 on both Host values, autoindex lists the drop, PUT/POST 403, `Range` 206 (resume works), `../` out of the drop 404. Scope was HTTP only; the runbook's optional Samba instance was skipped as it adds nothing for remote peers. Still to do before this unblocks `docs/runbooks/blaney/01-install-mw2-iw4x.md`: **put the MW2 payload in the drop** — that is a gaming-pc job (`tar -C /mnt/games -cf /mnt/storage/public/mw2.tar mw2`), and the blaney runbook plus the "Reaching the media" section of `mw2-iw4x-source.md` still say the media arrives on a USB drive.
