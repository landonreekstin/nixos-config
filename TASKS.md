...
- [ ] **Public file drop on optiplex-nas, for remote VPN users** *(do this on optiplex-nas; runbook: [docs/runbooks/nas-public-share.md](docs/runbooks/nas-public-share.md))* — Restricted VPN peers have no way to receive a file from the homelab. Blocking Blaney's MW2 install: he is in another state, so a USB drive is not an option and ~15 GB has to cross the WAN. Serve `/mnt/storage/public` read-only over **nginx**, not Samba — port 80 to the NAS is *already* in the restricted-peer pf allow-list so **no firewall change is needed**, HTTP resumes where CIFS does not, and read-only is inherent to serving GET. The trap that decides whether it works: restricted peers use 1.1.1.1 and cannot resolve `.lan`, so the vhost needs `serverAliases = [ "192.168.1.76" ]` or they hit whichever vhost nginx picked as default. Unblocks `docs/runbooks/blaney/01-install-mw2-iw4x.md`, which currently stops on its own prerequisite check.
