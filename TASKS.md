...
- [ ] **Host `landon` and `Venator class XvX` as Bedrock servers** *(run this one on mini-server — PR #140 open, terrain recovery still outstanding)* — Two Minecraft worlds recovered from a 2013 iPod / iPhone 5c, to be served as independently start/stoppable BDS instances with their own game-control dashboard tiles. Worlds are already staged at `mini-server:~/minecraft-worlds/`. Full runbook incl. the trap that makes this non-obvious (BDS 1.26 silently cannot read the original LegacyTerrain format and serves a regenerated empty world instead): `docs/runbooks/minecraft-bedrock-servers.md`. Also resolves a pre-existing inconsistency: `minecraft-bedrock` is defined both by the repo OCI container and by an unmanaged `docker-compose.yml` on the host, with mismatched `/data` mounts and a colliding container name.

- [ ] **Recover `landon` / `venator` terrain — mcpelauncher 1.14 + BDS 1.26 lost the world state** *(run this one on mini-server)* — Server side is now correct (PR #140 landed the two BDS instances with RakNet transport, declarative allowlist, dashboard tiles, firewall forwards); the outstanding item is that the runbook's terrain-verification step failed. When landon opened in BDS 1.26 it wrote nonsense metadata (`SpawnY=32767`, `limitedWorldWidth/Depth=16` on a 256×256 world) and treats the entire (0–255, 0–255) area — where all structures live per the runbook — as "outside of the world". Player spawns at (~580, 150, -105) in fresh procedural terrain instead. Same failure likely on venator (not re-tested; earlier session's clean-log verification proved only that BDS opened it, not that terrain is readable). Recovery paths: (1) restore pristine from `gaming-pc:~/Games/Minecraft-worlds/`, run through **chunker.app** (Java-based cross-version converter) before letting BDS touch it — the mcpelauncher-1.14 conversion path already failed, so trying it again unchanged won't help; (2) manually rewrite `level.dat` NBT to drop the `LimitedWorld*` markers and coerce `Generator=1` (infinite) before first BDS open; (3) newer mcpelauncher version as an intermediate 1.14→1.26 hop. Any attempt must start by wiping `/var/lib/game-servers/minecraft-bedrock-{landon,venator}/worlds/` — BDS's one-way upgrade already ran, so the current on-disk copies are trashed. XUID `2535408372064922` is already wired into the allowlist so a working recovery is immediately joinable.
