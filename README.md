...
Every push to `main` or `update/*` and every PR triggers a full evaluation of all 10 host configurations:

```yaml
hosts=(gaming-pc optiplex blaney-pc justus-pc asus-laptop asus-m15 atl-mini-pc optiplex-nas mini-server aj-laptop)
...
2. Builds all 10 hosts with per-host timeout tracking; results are posted as a Markdown table in the PR body
...
| `aj-laptop` | AJ's ROG Zephyrus G14 laptop |
